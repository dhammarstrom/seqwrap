...
```{r}
#| label: packages
library(tidyverse); library(glmmTMB); library(parallel)
...
# Remove missning values from the meta data 
...
fits <- seqwrap(fitting_fun = glmmTMB::glmmTMB,
                          arguments = args,
...
fits2 <- seqwrap(    fitting_fun = glmmTMB::glmmTMB,
                          arguments = args,
                          data = rna_seq_sample_integer,
                          metadata = rna_seq_metadata,
                          samplename = "seq_sample_id",
...
eval_mod <- function(x) {
  ...
  results <- data.frame(pval.unif = unif$p.value, 
...
fits3 <- seqwrap(    fitting_fun = glmmTMB::glmmTMB,
                          arguments = args,
                          data = rna_seq_sample_integer,
                          metadata = rna_seq_metadata,
                          samplename = "seq_sample_id",
...
bind_rows(fits3$summaries) %>%
  ...
  ggplot(aes(Estimate, -log(Pr...z..))) + geom_point(aes(color = pval.unif)) + facet_wrap(~ coef)
